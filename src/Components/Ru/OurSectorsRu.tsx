...
const sectorsHtml = sectors.map((sector, index) => (
    <div key={index} className="bg-[#fff] bg-opacity-[0.12] rounded-2xl shadow-[4px_4px_4px_0px_rgba(0,223,192,1)] md:w-[20%] w-[45%] aspect-square flex flex-col items-center p-2 pb-4 mb-6">
        <img src={`/OurSectorsImgs/sector${index+1}.webp`} alt={`sector${index+1} img`} className="mb-6 h-[80px] w-[80px] " />
        <h1 className="text-[#FFF] font-bold text-sm  md:text-lg 2xl:text-2xl text-center">{sector}</h1>
    </div>
));
